...
// 静态搜索数据，共15条
const searchData = new Array(15).fill(0).map((t, i) => {
  return  {
    address: '',
    latitude: 0,
    longitude: 0,
    city: '武汉',
    bus: `共计15条数据，我是第${i + 1}条`
  }
})
...
module.exports = {
  ...
  searchAddress: function (req, res) {
    let {latitude, longitude, keyword, offset, limit, name} = req.query
    let retData = []
    for(let i = offset * limit, j = 0; i < searchData.length && j < limit; ++i, ++j) {
      let tmp = Object.assign({}, searchData[i])
      tmp.address = `搜索关键词：${keyword}，搜索城市坐标：[${latitude},${longitude}]， 分页信息：${offset}:${limit}`
      tmp.city = name || tmp.city
      retData.push(tmp)
    }
